...
class AppHospede:
    def __init__(self, abaHospede, Init):
        ...
        self.Init = Init
        self.abaHospede = abaHospede

        #menu de funções
        self.frame_menu = Frame(self.abaHospede, bg="#00767d")
        ...
        self.btn_adicionar.bind("<Button-1>", self.formularioHospede)
        ...
        self.btn_busca = Button(self.frame_menu, text="BUSCAR", bg="#ffffff", fg= "#000000", command=lambda:[buscarHospedes()])
        self.btn_busca.place(relx= 0.8, rely= 0, relwidth= 0.2, relheight= 1)
        def buscarHospedes():
            HospedeController.buscarHospedes(self)
        HospedeController.buscarHospedes(self)

    def listarHospedes(self):
        #configuração da barra de rolagem
        self.f_base_scroll = Frame(self.abaHospede)
        self.f_base_scroll.place(relx= 0, rely= 0.09, relwidth= 1, relheight= 0.95)
        self.j_hospedes = Frame(self.f_base_scroll, bg="#00767d")
        self.canva_rol = Canvas(self.j_hospedes, bg="#00767d")
        self.canva_rol.place(relx= 0, rely= 0, relwidth= 1, relheight= 1)
        self.yscrollbar = ttk.Scrollbar(self.j_hospedes, orient="vertical", command=self.canva_rol.yview)
        ...
        self.j_hospedes.place(relx= 0, rely= 0, relwidth= 1, relheight= 1)
        
        #for para criar frames dos hospedes
        indice = 0
        ...
        def callDeletarHospede(event):
            if(messagebox.askquestion("Confirmar","Você tem certeza que quer deletar este usuário?")=="yes"):
                HospedeController.deletarHospede(event.widget["text"].split()[0],self.Init)

        self.img_hospede = []

        for row in range(math.ceil(len(self.rHospedes)/quadros)):
            quadros = len(self.rHospedes)%quadros if row==math.ceil(len(self.rHospedes)/quadros)-1 and len(self.rHospedes)%quadros!=0 == 0 else quadros
            self.estrutura = Frame(self.j_hospedes, bg="#00767d")
            self.estrutura.pack(in_=self.frame_rol, side=TOP, ipadx=0.47*self.Init.monitor_w, ipady=200, padx=10, pady=20)
            ...
            for col in range(quadros):
                ...
                self.img_hospede.append(PhotoImage(file=f"{self.Init.pasta_app}/imagens/{self.rHospedes[indice][9]}"))
                self.img_hospede[indice] = self.img_hospede[indice].subsample(3,3)
                self.label_img_hospede = Label(self.temp_frame, image=self.img_hospede[indice], bg="#002e4f", highlightbackground="#004170", highlightthickness= 2)
                self.label_img_hospede.place(relx= 0.68, rely= 0.02, relwidth= 0.3, relheight= 0.3)

                labels = ["Nome", "Cpf", "Rg", "Email", "Senha", "Telefone", "Data de Nascimento", "Sexo", "Foto_perfil", "Endereço", "Dados Bancários", "Data de Criação"]
                for i in range(len(labels)):
                    Label(self.temp_frame, text=f"{labels[i]}: {str(self.rHospedes[indice][i+1])}", bg="#ffffff", fg= "#000000", font=tkFont.Font(family="Lucida Grande", size=11)).place(relx=0.07, y=340/len(labels)*i+10, height=30)
                
                btn_editar = Label(self.temp_frame, text=f"{self.rHospedes[indice][0]}  Editar  {self.rHospedes[indice][0]}",  bg="blue", fg= "#ffffff", font=tkFont.Font(family="Lucida Grande", size=13))
                btn_editar.place(x= 5, rely= 0.9, width= 60, height= 30)
                btn_editar.bind("<Button-1>", self.formularioHospede)

                btn_deletar = Label(self.temp_frame, text=f"{self.rHospedes[indice][0]}  Deletar  {self.rHospedes[indice][0]}",  bg="red", fg= "#ffffff", font=tkFont.Font(family="Lucida Grande", size=13))
                btn_deletar.place(x=70, rely= 0.9, width= 60, height= 30)
                btn_deletar.bind("<Button-1>", callDeletarHospede)

                indice+=1

    def formularioHospede(self, event):
        #puxando dados
        acao = event.widget["text"].split()[1]
        if acao=="Editar":
            self.idhospede = event.widget["text"].split()[0]
            ...
            self.foto_perfil = self.dados_hospede[8]
        ...
        #definindo título
        self.label_titulo = Label(self.root2, text=f"{acao} Hóspede", font=tkFont.Font(family="Lucida Grande", size=15), bg="#283d8f", fg= "#ffffff")
        ...
        def callHospedeController():
            if acao=="Editar":
                HospedeController.alterarHospede(self, self.Init)
                print(acao, self.idhospede, nome)
            else:
                HospedeController.cadastrarHospede(self, self.Init)
        self.btn_salvar = Button(self.root2, text="SALVAR", bg="#ffffff", fg= "#000000", command=lambda:[callHospedeController()])
